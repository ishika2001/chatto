...
import { resultSearch } from "../../constants/resultSearch";
...
const LeftSidebar = ({
    ...
    const handleClick = () => {
        setValue(!value);
        collapsedSidebar && value ? disablePageScroll() : enablePageScroll();
    };
    ...
    return (
        <>
            <div
                className={twMerge(
                    `fixed z-20 top-0 left-0 bottom-0 flex flex-col pt-30 px-4 bg-n-7 md:invisible md:opacity-0 md:transition-opacity ${value
            ...
            >
                <div
                    className={`absolute top-0 right-0 left-0 flex items-center h-30 pl-7 pr-6 ${value ? "justify-center md:px-4" : "justify-between"
                        }`}
                >
                    {!value && <Logo />}
                    <button
